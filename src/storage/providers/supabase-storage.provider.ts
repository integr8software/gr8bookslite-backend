...
import { buildSafeFileName, encodeStoragePath, sanitizeRelativePath, validateStorageFolder } from '../storage-path.util';
...
export class SupabaseStorageProvider implements StorageProvider {
  ...
  async uploadFile(input: StorageUploadInput): Promise<StorageUploadResult> {
    ...
    const response = await fetch(`${this.getSupabaseUrl()}/storage/v1/object/${this.getBucket()}/${encodeStoragePath(relativePath)}`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new BadGatewayException(failureText || 'Supabase storage upload failed.');
    }
  ...
  async deleteFile(relativePath: string | null | undefined) {
    ...
    const response = await fetch(`${this.getSupabaseUrl()}/storage/v1/object/${this.getBucket()}/${encodeStoragePath(storagePath)}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${this.getServiceRoleKey()}`,
        apikey: this.getServiceRoleKey(),
    ...
    if (!response.ok && response.status !== 404) {
      ...
      throw new BadGatewayException(failureText || 'Supabase storage delete failed.');
    }
  }
  ...
  async moveFile(params: { sourcePath: string; destinationPath: string }) {
    ...
    const response = await fetch(`${this.getSupabaseUrl()}/storage/v1/object/move`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${this.getServiceRoleKey()}`,
        apikey: this.getServiceRoleKey(),
    ...
    if (!response.ok) {
      ...
      throw new BadGatewayException(failureText || 'Supabase storage move failed.');
    }
