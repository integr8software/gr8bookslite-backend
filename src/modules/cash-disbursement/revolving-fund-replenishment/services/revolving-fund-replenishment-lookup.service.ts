...
@Injectable()
export class RevolvingFundReplenishmentLookupService {
  ...
  async findRevolvingFunds(user: AuthUser) {
    const companyId = await this.getAccessibleCompanyId(user);
    const funds = await this.prisma.revolvingFund.findMany({
      where: {
        companyId,
        deletedAt: null,
        status: { in: [RevolvingFundStatus.APPROVED, RevolvingFundStatus.POSTED] },
      },
      orderBy: [{ documentDate: 'desc' }, { transactionNo: 'desc' }],
      include: {
        party: true,
    ...
    return {
      funds: funds.map((f) => ({
        ...
        documentDate: f.documentDate.toISOString().slice(0, 10),
        partyId: f.partyId?.toString() ?? null,
        partyCode: f.party?.partyCodeNo ?? f.partyCodeSnapshot,
        partyName: f.party?.partyName ?? f.partyNameSnapshot,
        amount: Number(f.amount),
        responsibilityCenterId: f.responsibilityCenterId?.toString() ?? null,
        responsibilityCenterCode: f.responsibilityCenter?.code ?? f.responsibilityCenterCodeSnapshot,
        responsibilityCenter: f.responsibilityCenter?.name ?? f.responsibilityCenterSnapshot,
        remarks: f.remarks,
      })),
    };
  }
  ...
  private async getAccessibleCompanyId(user: AuthUser): Promise<number> {
    ...
    if (!canAccessModuleAction(user, companyId, RevolvingFundReplenishmentModuleCode, PermissionAction.VIEW)) {
      throw new ForbiddenException('You do not have permission to view Revolving Fund Replenishment options.');
    }
    return companyId;
  }
}
