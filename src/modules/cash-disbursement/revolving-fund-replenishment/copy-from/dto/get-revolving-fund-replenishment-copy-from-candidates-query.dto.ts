...
export type RevolvingFundReplenishmentCopyFromTarget = (typeof TargetVoucherValues)[number];

export class GetRevolvingFundReplenishmentCopyFromCandidatesQueryDto {
  @ApiProperty({ enum: TargetVoucherValues, example: 'cash-voucher', description: 'Target voucher type' })
  @IsIn(TargetVoucherValues)
  target: RevolvingFundReplenishmentCopyFromTarget;

  @ApiPropertyOptional({ description: 'Search term across transaction number, party, or remarks', example: 'RFR-2026' })
  @IsOptional()
  @IsString()
  @MaxLength(120)
  search?: string;
