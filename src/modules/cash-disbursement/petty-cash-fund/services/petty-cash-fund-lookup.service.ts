...
@Injectable()
export class PettyCashFundLookupService {
  ...
  private async getAccessibleCompanyId(user: AuthUser): Promise<number> {
    ...
    if (!canAccessModuleAction(user, companyId, PettyCashFundModuleCode, PermissionAction.VIEW)) {
      throw new ForbiddenException('You do not have permission to view Petty Cash Fund options.');
    }
    return companyId;
  }
}
