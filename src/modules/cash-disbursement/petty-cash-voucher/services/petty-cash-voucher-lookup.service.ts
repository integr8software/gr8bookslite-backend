...
@Injectable()
export class PettyCashVoucherLookupService {
  ...
  private async getAccessibleCompanyId(user: AuthUser): Promise<number> {
    ...
    if (!canAccessModuleAction(user, companyId, PettyCashVoucherModuleCode, PermissionAction.VIEW)) {
      throw new ForbiddenException('You do not have permission to view Petty Cash Voucher options.');
    }
    return companyId;
  }
}
