...
export function mapDisbursementVoucher(voucher: DisbursementVoucherWithDetails, userNames: Map<number, string>) {
  ...
  const sourceDetails = (voucher.details || []).filter((detail) => !isGeneratedDisbursementVoucherDetail(detail));
  const grossAmount = sourceDetails.reduce((sum, detail) => sum + getDisbursementVoucherDetailGrossAmount(detail), 0);
  const disburseAmount = sourceDetails.reduce((sum, detail) => sum + getDisbursementVoucherDetailDisburseAmount(detail), 0);
  ...
  return {
    ...
    paymentDetails: voucher.paymentDetails ?? createEmptyPaymentDetails(),
    ...
    amount: roundDisbursementVoucherAmount(grossAmount || Number(voucher.amount)),
    disburseAmount: roundDisbursementVoucherAmount(disburseAmount || Number(voucher.amount)),
...
function getDisbursementVoucherDetailGrossAmount(detail: DisbursementVoucherWithDetails['details'][number]) {
...
function getDisbursementVoucherDetailDisburseAmount(detail: DisbursementVoucherWithDetails['details'][number]) {
  const grossAmount = getDisbursementVoucherDetailGrossAmount(detail);
...
function isGeneratedDisbursementVoucherDetail(detail: DisbursementVoucherWithDetails['details'][number]) {
...
function roundDisbursementVoucherAmount(value: number) {
