...
import type { DisbursementVoucherDetail } from '@prisma/client';
import type { DisbursementVoucherDetailDto } from '../dto/disbursement-voucher-detail.dto';
import type { JournalEntryDto } from '../dto/journal-entry.dto';
import type { DisbursementVoucherJournalEntry } from '../types/disbursement-voucher-with-details.type';
import { amountsMatch, getDisbursementVoucherDetailTotals, getJournalEntryTotals, roundCurrency } from '../utils/disbursement-voucher-totals.util';

export const DisbursementVoucherReferenceType: string = 'DV';

@Injectable()
export class DisbursementVoucherAccountingService {
  ...
  validatePersistedPayload({
    ...
    amount: number;
    details: DisbursementVoucherDetail[];
    journalEntries: DisbursementVoucherJournalEntry[];
  }) {
    const sourceDetails = details.filter(isPersistedSourceDetailRow);

    if (sourceDetails.length === 0) {
      throw new BadRequestException('Add at least one Disbursement Voucher detail row before approval.');
    }

    if (amount <= 0) {
      throw new BadRequestException('Disbursement Voucher amount must be non-zero before approval.');
    }
    ...
    if (journalEntries && journalEntries.length > 0) {
      ...
      for (const entry of journalEntries) {
        if (entry.referenceType !== DisbursementVoucherReferenceType) {
          throw new BadRequestException(`Disbursement Voucher journal rows must use referenceType ${DisbursementVoucherReferenceType}.`);
        }
  ...
  private validateDetailRows(details: DisbursementVoucherDetailDto[]) {
    const sourceDetails = details.filter(isSourceDetailRow);

    if (sourceDetails.length === 0) {
      throw new BadRequestException('Add at least one Disbursement Voucher detail row.');
    }
  ...
  private validateJournalRows(journalEntries: JournalEntryDto[], currencyCode: string, exchangeRate: number) {
    ...
    for (const entry of journalEntries) {
      if (entry.referenceType && entry.referenceType !== DisbursementVoucherReferenceType) {
        throw new BadRequestException(`Disbursement Voucher journal rows must use referenceType ${DisbursementVoucherReferenceType}.`);
      }
...
function isPersistedSourceDetailRow(detail: DisbursementVoucherDetail): boolean {
