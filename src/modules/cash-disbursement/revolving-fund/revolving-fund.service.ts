...
import { RevolvingFundModuleCode } from './services/revolving-fund-lookup.service';
...
export class RevolvingFundService {
  ...
  async findAll(user: AuthUser, query: GetRevolvingFundListQueryDto) {
    ...
    return {
      items: records.map((record) => RevolvingFundMapper.toResponseDto(record as any)),
      meta: {
        page,
        limit,
        total,
  ...
  async findOne(user: AuthUser, id: string) {
    ...
    return RevolvingFundMapper.toResponseDto(record as any);
  }
  ...
  async suggestTransactionNumber(user: AuthUser, branchUnitId?: number) {
    ...
    try {
      ...
      const count = await this.prisma.revolvingFund.count({ where: { companyId } });
      const sequenceNumber = (count + 1).toString().padStart(6, '0');
      const year = new Date().getFullYear();
      const nextTransNo = `RF-${year}-${sequenceNumber}`;
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      try {
        ...
        if (inputNo) {
          ...
          const count = await tx.revolvingFund.count({ where: { companyId } });
          const sequenceNumber = (count + 1).toString().padStart(6, '0');
          const year = new Date().getFullYear();
          assignedNo = `RF-${year}-${sequenceNumber}`;
        }
      }
      ...
      const created = await tx.revolvingFund.create({
        data: {
          ...
          status: dto.status ?? (RevolvingFundStatus.DRAFT as any),
      ...
      return RevolvingFundMapper.toResponseDto(reloaded as any);
    });
  }
  ...
  async update(user: AuthUser, id: string, dto: UpdateRevolvingFundDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (dto.details !== undefined) {
        await tx.revolvingFundDetail.deleteMany({ where: { fundId: recordId } as any });
  ...
  async updateStatus(user: AuthUser, id: string, dto: UpdateRevolvingFundStatusDto) {
    ...
    return RevolvingFundMapper.toResponseDto(updated as any);
  }
