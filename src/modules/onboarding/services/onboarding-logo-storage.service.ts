...
export class OnboardingLogoStorageService {
  ...
  async uploadLogo(params: {
    ...
    const storagePath = `onboarding/user-${params.userId}/${Date.now()}-${safeFileName}`;
  ...
  async moveLogo(params: { sourcePath: string; destinationPath: string }) {
    ...
    const response = await fetch(`${this.supabaseUrl}/storage/v1/object/move`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${this.serviceRoleKey}`,
        apikey: this.serviceRoleKey,
      ...
      body: JSON.stringify({
        ...
        sourceKey: params.sourcePath,
        destinationKey: params.destinationPath,
      }),
    });
    ...
    if (!response.ok) {
      ...
      throw new BadGatewayException(
        failureText || 'Supabase logo move failed.',
      );
    }

    return {
      storagePath: params.destinationPath,
      publicUrl: this.buildPublicUrl(params.destinationPath),
    };
  }
