...
export const TermsMaintenanceSeedRecords = [
...
export async function seedCompanyTermsMaintenanceDefaults(tx: TermWriteClient, companyId: number) {
  const existingTerms = await tx.term.findMany({
    where: {
      companyId,
      name: {
        in: TermsMaintenanceSeedRecords.map((term) => term.name),
      },
    },
    select: { name: true },
  });
