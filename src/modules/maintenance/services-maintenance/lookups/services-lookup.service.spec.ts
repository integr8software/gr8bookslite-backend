...
describe('ServicesLookupService', () => {
  it('returns searchable active service options with the shared name field', async () => {
    ...
    prisma.serviceMaintenance.findMany.mockResolvedValue([{ id: 14n, serviceName: 'Consulting', status: ChartAccountStatus.ACTIVE }]);
    ...
    expect(result).toEqual([{ id: '14', serviceName: 'Consulting', name: 'Consulting', status: ChartAccountStatus.ACTIVE }]);
  });
});
