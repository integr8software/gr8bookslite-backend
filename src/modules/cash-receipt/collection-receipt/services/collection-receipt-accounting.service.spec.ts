...
import { CollectionReceiptAccountingService } from './collection-receipt-accounting.service';

describe('CollectionReceiptAccountingService', () => {
  const service = new CollectionReceiptAccountingService();
  const createPayload = () => {
    const detail: CollectionReceiptDetailDto = {
      lineNumber: 1,
    ...
    ].map((entry, index) => ({ ...entry, lineNumber: index + 1, referenceType: 'CR', currencyCode: 'PHP', exchangeRate: 1 }));
