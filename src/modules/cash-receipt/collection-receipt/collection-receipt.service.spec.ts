...
import type { CollectionReceiptAccountingService } from './services/collection-receipt-accounting.service';

describe('CollectionReceiptService payment type resolution', () => {
  const findFirst = jest.fn();
  const prisma = {
    paymentType: { findFirst },
  } as unknown as PrismaService;
  const service = new CollectionReceiptService(prisma, {} as CollectionReceiptAccountingService);
...
describe('CollectionReceiptService entry persistence', () => {
  const service = new CollectionReceiptService({} as PrismaService, {} as CollectionReceiptAccountingService);

  it('stores each collection item and its hidden columns in CollectionReceiptDetails', async () => {
    const createMany = jest.fn().mockResolvedValue({ count: 1 });
    const tx = {
      collectionReceiptDetails: { deleteMany: jest.fn().mockResolvedValue({ count: 0 }), createMany },
    } as unknown as Prisma.TransactionClient;
    const input: CollectionReceiptDetailDto = {
      lineNumber: 1,
  ...
  it('stores accounting rows under JournalEntryHeader with nested JournalEntryDetail rows', async () => {
    ...
    expect(deleteMany).toHaveBeenCalledWith({ where: { referenceId: 7n, referenceType: 'CR' } });
