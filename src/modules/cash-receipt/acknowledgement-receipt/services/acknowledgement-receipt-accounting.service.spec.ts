...
import { AcknowledgementReceiptAccountingService } from './acknowledgement-receipt-accounting.service';

describe('AcknowledgementReceiptAccountingService', () => {
  const service = new AcknowledgementReceiptAccountingService();
  const createPayload = () => {
    const detail: AcknowledgementReceiptDetailDto = {
      lineNumber: 1,
    ...
    const journalEntries: AcknowledgementReceiptJournalEntryDto[] = [
    ...
    ].map((entry, index) => ({ ...entry, lineNumber: index + 1, referenceType: 'AR', currencyCode: 'PHP', exchangeRate: 1 }));
