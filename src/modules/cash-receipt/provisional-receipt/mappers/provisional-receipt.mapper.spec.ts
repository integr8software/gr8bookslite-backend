...
import { mapProvisionalReceipt } from './provisional-receipt.mapper';
import type { ProvisionalReceiptWithDetails } from '../types/provisional-receipt-with-details.type';

describe('mapProvisionalReceipt', () => {
  it('maps payment details, monetary values, and journal snapshots for the API', () => {
    const receipt = {
      ...
      receiptNo: 'PVR-2026-0006',
      referenceNo: 'REF-001',
      remarks: null,
      status: ProvisionalReceiptStatus.DRAFT,
      transactionNo: 'PVR-2026-0006',
    ...
    } as unknown as ProvisionalReceiptWithDetails;

    const mapped = mapProvisionalReceipt(
      receipt,
      new Map([
        [7, 'Creator User'],
        [8, 'Updater User'],
    ...
    expect(mapped.details[0]).not.toHaveProperty('discountAmount');
    expect(mapped.details[0]).not.toHaveProperty('wvatAmount');
