...
import { amountsMatch, getJournalEntryTotals, getProvisionalReceiptDetailTotals } from '../utils/provisional-receipt-totals.util';

const ProvisionalReceiptReferenceType = 'PVR';

@Injectable()
export class ProvisionalReceiptAccountingService {
  validateSubmittedPayload({
    ...
    const detailTotals = getProvisionalReceiptDetailTotals(details);
  ...
  private validateJournalRows(journalEntries: ProvisionalReceiptJournalEntryDto[], currencyCode: string, exchangeRate: number) {
    if (journalEntries.length < 2) {
