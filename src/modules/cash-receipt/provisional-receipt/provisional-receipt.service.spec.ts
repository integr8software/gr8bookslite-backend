...
import type { ProvisionalReceiptAccountingService } from './services/provisional-receipt-accounting.service';

describe('ProvisionalReceiptService payment type resolution', () => {
  const findFirst = jest.fn();
  const prisma = {
    paymentType: { findFirst },
  } as unknown as PrismaService;
  const service = new ProvisionalReceiptService(prisma, {} as ProvisionalReceiptAccountingService);
...
describe('ProvisionalReceiptService entry persistence', () => {
  const service = new ProvisionalReceiptService({} as PrismaService, {} as ProvisionalReceiptAccountingService);

  it('stores each collection item and its hidden columns in ProvisionalReceiptDetails', async () => {
    const createMany = jest.fn().mockResolvedValue({ count: 1 });
    const tx = {
      provisionalReceiptDetails: { deleteMany: jest.fn().mockResolvedValue({ count: 0 }), createMany },
    } as unknown as Prisma.TransactionClient;
    const input: ProvisionalReceiptDetailDto = {
      lineNumber: 1,
  ...
  it('stores accounting rows under JournalEntryHeader with nested JournalEntryDetail rows', async () => {
    ...
    expect(deleteMany).toHaveBeenCalledWith({ where: { referenceId: 7n, referenceType: 'PVR' } });
