...
import { amountsMatch, getJournalEntryTotals, getOfficialReceiptDetailTotals } from '../utils/official-receipt-totals.util';

const OfficialReceiptReferenceType = 'OR';

@Injectable()
export class OfficialReceiptAccountingService {
  validateSubmittedPayload({
    ...
    const detailTotals = getOfficialReceiptDetailTotals(details);
  ...
  validatePersistedPayload({ details, journalEntries }: { details: OfficialReceiptDetails[]; journalEntries: OfficialReceiptJournalEntry[] }) {
    if (details.length === 0) {
      throw new BadRequestException('Add at least one official receipt item before posting.');
    }
    ...
    for (const entry of journalEntries) {
      if (entry.referenceType !== OfficialReceiptReferenceType) {
        throw new BadRequestException('official receipt journal rows must use referenceType OR.');
      }
  ...
  private validateDetailRows(details: OfficialReceiptDetailDto[]) {
    if (details.length === 0) {
      throw new BadRequestException('Add at least one official receipt item.');
    }
  ...
  private validateJournalRows(journalEntries: OfficialReceiptJournalEntryDto[], currencyCode: string, exchangeRate: number) {
    if (journalEntries.length < 2) {
      throw new BadRequestException('Add at least two journal entry rows.');
    }

    for (const entry of journalEntries) {
      if (entry.referenceType && entry.referenceType !== OfficialReceiptReferenceType) {
        throw new BadRequestException('official receipt journal rows must use referenceType OR.');
      }
