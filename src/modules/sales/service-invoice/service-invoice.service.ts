...
import { ServiceInvoiceJournalEntryDto } from './dto/service-invoice-journal-entry.dto';
...
import { mapServiceInvoice } from './mappers/service-invoice.mapper';
...
const ServiceInvoiceModuleCode = 'SVI';
...
export class ServiceInvoiceService {
  constructor(
    ...
    private readonly accountingService: ServiceInvoiceAccountingService,
  ) {}

  async findAll(user: AuthUser, query: GetServiceInvoiceListQueryDto) {
  ...
  async suggestTransactionNumber(user: AuthUser, requestedBranchUnitId?: number) {
    ...
    const suggestion = await suggestTransactionNumberForCompanyBranch(this.prisma, {
      branchUnitId,
      companyId,
      moduleCode: ServiceInvoiceModuleCode,
  ...
  async create(user: AuthUser, dto: CreateServiceInvoiceDto) {
    const companyId = this.getActiveCompanyId(user);
    await this.ensureCompanyAccess(user, companyId);
    this.ensureCan(user, companyId, PermissionAction.CREATE);
    ...
    try {
      const invoice = await this.prisma.$transaction(async (tx) => {
        ...
        const saved = await tx.serviceInvoice.findUniqueOrThrow({ where: { id: created.id }, include: ServiceInvoiceInclude });
  ...
  async update(user: AuthUser, id: string, dto: UpdateServiceInvoiceDto) {
    const companyId = this.getActiveCompanyId(user);
    await this.ensureCompanyAccess(user, companyId);
    this.ensureCan(user, companyId, PermissionAction.UPDATE);
    const serviceInvoiceId = parsePositiveBigIntId(id);
    const current = await this.findInvoiceOrThrow(companyId, serviceInvoiceId);

    if (current.status !== ServiceInvoiceStatus.DRAFT) {
      throw new BadRequestException('Only draft service invoices can be edited.');
    }

    if (dto.branchUnitId !== undefined && dto.branchUnitId !== current.branchUnitId) {
      throw new BadRequestException('Service invoice branch cannot be changed after creation.');
    }
    ...
    try {
      const invoice = await this.prisma.$transaction(async (tx) => {
        ...
        const transactionNo = await this.resolveTransactionNumberForUpdate(tx, {
          ...
          excludedInvoiceId: serviceInvoiceId,
  ...
  async updateStatus(user: AuthUser, id: string, dto: UpdateServiceInvoiceStatusDto) {
    const companyId = this.getActiveCompanyId(user);
    await this.ensureCompanyAccess(user, companyId);
    const targetStatus = this.normalizeStatus(dto.status);
    const requiredAction = targetStatus === ServiceInvoiceStatus.CANCELLED ? PermissionAction.CANCEL : PermissionAction.UPDATE;
    ...
    if (current.status === targetStatus) {
      return {
        ...
        message: 'Service invoice status is already up to date.',
        permissions: this.getPermissions(user, companyId),
      };
    }
    ...
    if (targetStatus === ServiceInvoiceStatus.POSTED) {
    ...
    const invoice = await this.prisma.serviceInvoice.update({
      where: { id: serviceInvoiceId },
      data: {
    ...
    return {
      ...
      message: 'Service invoice status updated successfully.',
      permissions: this.getPermissions(user, companyId),
    };
  }

  private buildListWhere(companyId: number, branchUnitId: number, query: GetServiceInvoiceListQueryDto): Prisma.ServiceInvoiceWhereInput {
  ...
  private buildOrderBy(query: GetServiceInvoiceListQueryDto): Prisma.ServiceInvoiceOrderByWithRelationInput[] {
  ...
  private getStatistics(companyId: number, branchUnitId: number) {
    return this.prisma.serviceInvoice
      .groupBy({
        by: ['status'],
      ...
      .then((groups) => {
        ...
        for (const group of groups) {
          ...
          if (group.status === ServiceInvoiceStatus.CANCELLED) statistics.cancelledInvoices += count;
          if (group.status === ServiceInvoiceStatus.DISAPPROVED) statistics.disapprovedInvoices += count;
          if (group.status === ServiceInvoiceStatus.DRAFT) statistics.draftInvoices += count;
          if (group.status === ServiceInvoiceStatus.FOR_APPROVAL) statistics.forApprovalInvoices += count;
          if (group.status === ServiceInvoiceStatus.POSTED) statistics.postedInvoices += count;
        }
  ...
  private async mapInvoicesWithAuditUsers(invoices: ServiceInvoiceWithDetails[]) {
    const userNames = await resolveAuditUserNames(
      this.prisma,
      invoices.flatMap((invoice) => [invoice.createdByUserId, invoice.updatedByUserId]),
    );

    return invoices.map((invoice) => mapServiceInvoice(invoice, userNames));
  }
  ...
  private async attachJournalEntries(
    invoices: Prisma.ServiceInvoiceGetPayload<{ include: typeof ServiceInvoiceInclude }>[],
    tx: PrismaWriteClient = this.prisma,
  ): Promise<ServiceInvoiceWithDetails[]> {
    ...
    const entriesByReferenceId = new Map<string, ServiceInvoiceWithDetails['journalEntries']>();
  ...
  private async resolveInvoiceReferences(tx: PrismaWriteClient, companyId: number, dto: CreateServiceInvoiceDto) {
  ...
  private async resolveDetailLine(tx: PrismaWriteClient, companyId: number, input: ServiceInvoiceDetailDto): Promise<ResolvedDetailLine> {
    return {
      input,
  ...
  private async resolveJournalEntry(tx: PrismaWriteClient, companyId: number, input: ServiceInvoiceJournalEntryDto): Promise<ResolvedJournalEntry> {
    return {
  ...
  private async replaceDetails(tx: Prisma.TransactionClient, serviceInvoiceId: bigint, companyId: number, branchUnitId: number, details: ResolvedDetailLine[]) {
    await tx.serviceInvoiceDetails.deleteMany({ where: { serviceInvoiceId } });
    await tx.serviceInvoiceDetails.createMany({
  ...
  private async resolveTransactionNumberForUpdate(
    ...
    if (sequence?.inputMode !== TransactionNumberInputMode.MANUAL) {
      throw new BadRequestException('Service invoice transaction number is auto-generated for this branch and cannot be changed manually.');
    }
  ...
  private async resolveParty(tx: PrismaWriteClient, companyId: number, { partyCode, partyId }: { partyCode?: string | null; partyId?: string | null }) {
    ...
    if (!party.partyTypes.some((partyType) => CustomerPartyTypes.has(partyType))) {
      throw new BadRequestException('Party must be a customer or member for service invoice.');
    }
  ...
  private async ensureTransactionNoAvailable(tx: PrismaWriteClient, companyId: number, branchUnitId: number, transactionNo: string, excludedInvoiceId?: bigint, scope: 'all' | 'branch' = 'branch') {
    ...
    if (existing) {
      throw new ConflictException('A service invoice with this transaction number already exists for this branch.');
    }
  }
  ...
  private ensureStatusTransitionAllowed(currentStatus: ServiceInvoiceStatus, targetStatus: ServiceInvoiceStatus) {
    const allowedStatuses: Record<ServiceInvoiceStatus, ServiceInvoiceStatus[]> = {
      [ServiceInvoiceStatus.CANCELLED]: [],
      [ServiceInvoiceStatus.DISAPPROVED]: [ServiceInvoiceStatus.DRAFT],
      [ServiceInvoiceStatus.DRAFT]: [ServiceInvoiceStatus.CANCELLED, ServiceInvoiceStatus.DISAPPROVED, ServiceInvoiceStatus.FOR_APPROVAL],
      [ServiceInvoiceStatus.FOR_APPROVAL]: [ServiceInvoiceStatus.CANCELLED, ServiceInvoiceStatus.DISAPPROVED, ServiceInvoiceStatus.POSTED],
      [ServiceInvoiceStatus.POSTED]: [],
    };

    if (!allowedStatuses[currentStatus].includes(targetStatus)) {
      throw new BadRequestException(`Cannot move service invoice from ${currentStatus} to ${targetStatus}.`);
    }
  }

  private getStatusAuditData(targetStatus: ServiceInvoiceStatus, userId: number): Prisma.ServiceInvoiceUncheckedUpdateInput {
    const now = new Date();

    if (targetStatus === ServiceInvoiceStatus.FOR_APPROVAL) {
      return { approvedAt: now, approvedByUserId: userId };
    }

    if (targetStatus === ServiceInvoiceStatus.DISAPPROVED) {
      return { disapprovedAt: now, disapprovedByUserId: userId };
    }

    if (targetStatus === ServiceInvoiceStatus.CANCELLED) {
  ...
  private normalizeInvoiceInput(dto: CreateServiceInvoiceDto) {
  ...
  private requireCompleteUpdateDto(dto: UpdateServiceInvoiceDto): CreateServiceInvoiceDto {
    const requiredFields: Array<keyof CreateServiceInvoiceDto> = [
      'customerCode',
      'customerName',
      'currency',
      'details',
    ...
    for (const field of requiredFields) {
      if (dto[field] === undefined || dto[field] === null) {
        throw new BadRequestException(`Field ${String(field)} is required when updating a service invoice.`);
      }
    }

    return dto as CreateServiceInvoiceDto;
  }
  ...
  private normalizeStatus(status: string) {
    ...
    if (!Object.values(ServiceInvoiceStatus).includes(normalized as ServiceInvoiceStatus)) {
      throw new BadRequestException('Invalid service invoice status.');
    }

    return normalized as ServiceInvoiceStatus;
  }
  ...
  private ensureCan(user: AuthUser, companyId: number, action: PermissionAction) {
    ...
    if (user.companyId === companyId && user.permissions.includes(`${ServiceInvoiceModuleCode}:${action}`)) {
      return;
    }

    throw new ForbiddenException('You do not have permission to manage service invoices.');
  }
  ...
  private can(user: AuthUser, companyId: number, action: PermissionAction) {
    ...
    return user.companyId === companyId && user.permissions.includes(`${ServiceInvoiceModuleCode}:${action}`);
  }
