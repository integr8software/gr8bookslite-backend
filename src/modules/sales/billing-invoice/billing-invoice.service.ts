...
import { ChartAccount, ChartAccountStatus, CompanyUnitType, MembershipRole, MembershipStatus, Party, PartyAddress, PartyStatus, PartyType, Prisma, ResponsibilityCenter, ResponsibilityCenterStatus, BillingInvoiceStatus, Term, TermStatus, TransactionNumberInputMode } from '@prisma/client';
...
import { BillingInvoiceDetailDto } from './dto/billing-invoice-detail.dto';
import { BillingInvoiceJournalEntryDto } from './dto/billing-invoice-journal-entry.dto';
...
import { mapBillingInvoice } from './mappers/billing-invoice.mapper';
...
import type { BillingInvoiceWithDetails } from './types/billing-invoice-with-details.type';

const BillingInvoiceModuleCode = 'BI';
...
export class BillingInvoiceService {
  constructor(
    ...
    private readonly accountingService: BillingInvoiceAccountingService,
  ) {}

  async findAll(user: AuthUser, query: GetBillingInvoiceListQueryDto) {
  ...
  async suggestTransactionNumber(user: AuthUser, requestedBranchUnitId?: number) {
    ...
    const suggestion = await suggestTransactionNumberForCompanyBranch(this.prisma, {
      branchUnitId,
      companyId,
      moduleCode: BillingInvoiceModuleCode,
  ...
  async create(user: AuthUser, dto: CreateBillingInvoiceDto) {
    const companyId = this.getActiveCompanyId(user);
    await this.ensureCompanyAccess(user, companyId);
    this.ensureCan(user, companyId, PermissionAction.CREATE);
    ...
    try {
      const invoice = await this.prisma.$transaction(async (tx) => {
        ...
        const saved = await tx.billingInvoice.findUniqueOrThrow({ where: { id: created.id }, include: BillingInvoiceInclude });
  ...
  async update(user: AuthUser, id: string, dto: UpdateBillingInvoiceDto) {
    const companyId = this.getActiveCompanyId(user);
    await this.ensureCompanyAccess(user, companyId);
    this.ensureCan(user, companyId, PermissionAction.UPDATE);
    const billingInvoiceId = parsePositiveBigIntId(id);
    const current = await this.findInvoiceOrThrow(companyId, billingInvoiceId);

    if (current.status !== BillingInvoiceStatus.DRAFT) {
      throw new BadRequestException('Only draft billing invoices can be edited.');
    }

    if (dto.branchUnitId !== undefined && dto.branchUnitId !== current.branchUnitId) {
      throw new BadRequestException('Billing invoice branch cannot be changed after creation.');
    }
    ...
    try {
      const invoice = await this.prisma.$transaction(async (tx) => {
        ...
        const transactionNo = await this.resolveTransactionNumberForUpdate(tx, {
          ...
          excludedInvoiceId: billingInvoiceId,
        ...
        await this.replaceDetails(tx, billingInvoiceId, companyId, current.branchUnitId, references.details);
        await this.replaceJournalEntries(tx, billingInvoiceId, companyId, current.branchUnitId, normalized.currencyCode, normalized.exchangeRate, references.journalEntries);

        const saved = await tx.billingInvoice.findUniqueOrThrow({ where: { id: billingInvoiceId }, include: BillingInvoiceInclude });
        return (await this.attachJournalEntries([saved], tx))[0];
      });
  ...
  async updateStatus(user: AuthUser, id: string, dto: UpdateBillingInvoiceStatusDto) {
    const companyId = this.getActiveCompanyId(user);
    await this.ensureCompanyAccess(user, companyId);
    const targetStatus = this.normalizeStatus(dto.status);
    const requiredAction = targetStatus === BillingInvoiceStatus.CANCELLED ? PermissionAction.CANCEL : PermissionAction.UPDATE;
    ...
    if (current.status === targetStatus) {
      return {
        ...
        message: 'Billing invoice status is already up to date.',
        permissions: this.getPermissions(user, companyId),
      };
    }
    ...
    if (targetStatus === BillingInvoiceStatus.POSTED) {
    ...
    return {
      ...
      message: 'Billing invoice status updated successfully.',
      permissions: this.getPermissions(user, companyId),
    };
  }

  private buildListWhere(companyId: number, branchUnitId: number, query: GetBillingInvoiceListQueryDto): Prisma.BillingInvoiceWhereInput {
  ...
  private buildOrderBy(query: GetBillingInvoiceListQueryDto): Prisma.BillingInvoiceOrderByWithRelationInput[] {
  ...
  private getStatistics(companyId: number, branchUnitId: number) {
    return this.prisma.billingInvoice
      .groupBy({
        by: ['status'],
      ...
      .then((groups) => {
        ...
        for (const group of groups) {
          ...
          if (group.status === BillingInvoiceStatus.CANCELLED) statistics.cancelledInvoices += count;
          if (group.status === BillingInvoiceStatus.DISAPPROVED) statistics.disapprovedInvoices += count;
          if (group.status === BillingInvoiceStatus.DRAFT) statistics.draftInvoices += count;
          if (group.status === BillingInvoiceStatus.FOR_APPROVAL) statistics.forApprovalInvoices += count;
          if (group.status === BillingInvoiceStatus.POSTED) statistics.postedInvoices += count;
        }
  ...
  private async mapInvoicesWithAuditUsers(invoices: BillingInvoiceWithDetails[]) {
    const userNames = await resolveAuditUserNames(
      this.prisma,
      invoices.flatMap((invoice) => [invoice.createdByUserId, invoice.updatedByUserId]),
    );

    return invoices.map((invoice) => mapBillingInvoice(invoice, userNames));
  }
  ...
  private async attachJournalEntries(
    invoices: Prisma.BillingInvoiceGetPayload<{ include: typeof BillingInvoiceInclude }>[],
    tx: PrismaWriteClient = this.prisma,
  ): Promise<BillingInvoiceWithDetails[]> {
    ...
    const journalHeaders = await tx.journalEntryHeader.findMany({
      where: {
        ...
        referenceType: BillingInvoiceReferenceType,
      },
      include: {
        details: {
    ...
    const entriesByReferenceId = new Map<string, BillingInvoiceWithDetails['journalEntries']>();
  ...
  private async resolveInvoiceReferences(tx: PrismaWriteClient, companyId: number, dto: CreateBillingInvoiceDto) {
  ...
  private async resolveDetailLine(tx: PrismaWriteClient, companyId: number, input: BillingInvoiceDetailDto): Promise<ResolvedDetailLine> {
    return {
      input,
  ...
  private async resolveJournalEntry(tx: PrismaWriteClient, companyId: number, input: BillingInvoiceJournalEntryDto): Promise<ResolvedJournalEntry> {
    return {
  ...
  private async replaceDetails(tx: Prisma.TransactionClient, billingInvoiceId: bigint, companyId: number, branchUnitId: number, details: ResolvedDetailLine[]) {
    await tx.billingInvoiceDetails.deleteMany({ where: { billingInvoiceId } });
    await tx.billingInvoiceDetails.createMany({
  ...
  private async resolveTransactionNumberForUpdate(
    ...
    if (sequence?.inputMode !== TransactionNumberInputMode.MANUAL) {
      throw new BadRequestException('Billing invoice transaction number is auto-generated for this branch and cannot be changed manually.');
    }
  ...
  private async resolveParty(tx: PrismaWriteClient, companyId: number, { partyCode, partyId }: { partyCode?: string | null; partyId?: string | null }) {
    ...
    if (!party.partyTypes.some((partyType) => CustomerPartyTypes.has(partyType))) {
      throw new BadRequestException('Party must be a customer or member for billing invoice.');
    }
  ...
  private async ensureTransactionNoAvailable(tx: PrismaWriteClient, companyId: number, branchUnitId: number, transactionNo: string, excludedInvoiceId?: bigint, scope: 'all' | 'branch' = 'branch') {
    ...
    if (existing) {
      throw new ConflictException('A billing invoice with this transaction number already exists for this branch.');
    }
  }
  ...
  private ensureStatusTransitionAllowed(currentStatus: BillingInvoiceStatus, targetStatus: BillingInvoiceStatus) {
    const allowedStatuses: Record<BillingInvoiceStatus, BillingInvoiceStatus[]> = {
      [BillingInvoiceStatus.CANCELLED]: [],
      [BillingInvoiceStatus.DISAPPROVED]: [BillingInvoiceStatus.DRAFT],
      [BillingInvoiceStatus.DRAFT]: [BillingInvoiceStatus.CANCELLED, BillingInvoiceStatus.DISAPPROVED, BillingInvoiceStatus.FOR_APPROVAL],
      [BillingInvoiceStatus.FOR_APPROVAL]: [BillingInvoiceStatus.CANCELLED, BillingInvoiceStatus.DISAPPROVED, BillingInvoiceStatus.POSTED],
      [BillingInvoiceStatus.POSTED]: [],
    };

    if (!allowedStatuses[currentStatus].includes(targetStatus)) {
      throw new BadRequestException(`Cannot move billing invoice from ${currentStatus} to ${targetStatus}.`);
    }
  }

  private getStatusAuditData(targetStatus: BillingInvoiceStatus, userId: number): Prisma.BillingInvoiceUncheckedUpdateInput {
    const now = new Date();

    if (targetStatus === BillingInvoiceStatus.FOR_APPROVAL) {
      return { approvedAt: now, approvedByUserId: userId };
    }

    if (targetStatus === BillingInvoiceStatus.DISAPPROVED) {
      return { disapprovedAt: now, disapprovedByUserId: userId };
    }

    if (targetStatus === BillingInvoiceStatus.CANCELLED) {
  ...
  private normalizeInvoiceInput(dto: CreateBillingInvoiceDto) {
  ...
  private requireCompleteUpdateDto(dto: UpdateBillingInvoiceDto): CreateBillingInvoiceDto {
    const requiredFields: Array<keyof CreateBillingInvoiceDto> = [
      'customerCode',
      'customerName',
      'currency',
      'details',
    ...
    for (const field of requiredFields) {
      if (dto[field] === undefined || dto[field] === null) {
        throw new BadRequestException(`Field ${String(field)} is required when updating a billing invoice.`);
      }
    }

    return dto as CreateBillingInvoiceDto;
  }
  ...
  private normalizeStatus(status: string) {
    ...
    if (!Object.values(BillingInvoiceStatus).includes(normalized as BillingInvoiceStatus)) {
      throw new BadRequestException('Invalid billing invoice status.');
    }

    return normalized as BillingInvoiceStatus;
  }
  ...
  private ensureCan(user: AuthUser, companyId: number, action: PermissionAction) {
    ...
    if (user.companyId === companyId && user.permissions.includes(`${BillingInvoiceModuleCode}:${action}`)) {
      return;
    }

    throw new ForbiddenException('You do not have permission to manage billing invoices.');
  }
  ...
  private can(user: AuthUser, companyId: number, action: PermissionAction) {
    ...
    return user.companyId === companyId && user.permissions.includes(`${BillingInvoiceModuleCode}:${action}`);
  }
