...
export class PurchaseOrderService {
  ...
  private async entries(companyId: number, branchUnitId: number, items: PurchaseOrderItemDto[], purchaseType: string, headerPrId?: bigint) {
    return Promise.all(items.map(async (item, index) => {
      ...
      const gross = roundMoney(item.poQty * item.price);
      const rate = Math.max(item.discountRate ?? 0, 0);
      const discount = roundMoney(gross * rate / 100);
      const after = roundMoney(Math.max(gross - discount, 0));
      const vatable = item.vatable ?? false;
      const vatInclusive = vatable && (item.vatInclusive ?? false);
      const vat = roundMoney(!vatable ? 0 : vatInclusive ? (after / 1.12) * 0.12 : after * 0.12);
      const netOfVat = roundMoney(vatInclusive ? Math.max(after - vat, 0) : after);
      const net = roundMoney(vatable && !vatInclusive ? after + vat : after);
      return { companyId, branchUnitId, lineNo: index + 1, purchaseRequestEntryId: prEntry?.id ?? null, responsibilityCenterId: rc?.id ?? null, serviceMaintenanceId: service?.id ?? null, itemId: cleanOptional(item.itemId), itemCode: cleanOptional(item.itemCode), barcode: cleanOptional(item.barcode), description, color: cleanOptional(item.color), brand: cleanOptional(item.brand), size: cleanOptional(item.size), model: cleanOptional(item.model), uom: cleanOptional(item.uom), lotNo: cleanOptional(item.lotNo), prQty: new Prisma.Decimal(item.prQty), poQty: new Prisma.Decimal(item.poQty), price: new Prisma.Decimal(item.price), grossAmount: new Prisma.Decimal(gross), discountRate: new Prisma.Decimal(rate), discountAmount: new Prisma.Decimal(discount), grossAfterDiscount: new Prisma.Decimal(after), vatAmount: new Prisma.Decimal(vat), vatable, vatInclusive, netOfVatAmount: new Prisma.Decimal(netOfVat), netAmount: new Prisma.Decimal(net), prNoSnapshot: cleanOptional(item.prNo), canvassNoSnapshot: cleanOptional(item.canvassNo), responsibilityCenterName: rc?.name ?? cleanOptional(item.responsibilityCenter) };
    }));
  }
