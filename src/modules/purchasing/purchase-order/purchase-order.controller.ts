...
export class PurchaseOrderController {
  ...
  findAll(@CurrentUser() user: AuthUser, @Query() query: GetPurchaseOrderListQueryDto) {
    return this.service.findAll(user, query);
  }
  @Get(':id')
  @ApiOperation({ summary: 'Get a purchase order' })
  @ApiOkResponse({ type: PurchaseOrderContainerResponseDto })
  findOne(@CurrentUser() user: AuthUser, @Param('id') id: string) {
    return this.service.findOne(user, id);
  }
  @Post()
  @ApiOperation({ summary: 'Create a purchase order' })
  @ApiCreatedResponse({ type: PurchaseOrderContainerResponseDto })
