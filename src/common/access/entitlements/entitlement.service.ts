...
import type { EnabledModuleRecord, EntitledModuleRecord, PlanModuleEntitlementSource } from './entitlement.types';
...
type EntitlementQueryClient = Pick<Prisma.TransactionClient, 'companySubscription'>;

type CompanyPlanModuleSubscription = Prisma.CompanySubscriptionGetPayload<{
  include: typeof CompanyPlanModulesInclude;
}>;

export type CompanyPlanModule = CompanyPlanModuleSubscription['plan']['systems'][number]['system']['modules'][number]['module'];

type CompanyPlanSidebarSubscription = Prisma.CompanySubscriptionGetPayload<{
  include: typeof CompanyPlanSidebarInclude;
}>;

export type CompanyPlanSidebarItem = CompanyPlanSidebarSubscription['plan']['systems'][number]['system']['sidebarItems'][number] & {
  systemCode: string;
};
...
export class EntitlementService {
  ...
  getEnabledModules<TEnabledModule extends EntitledModuleRecord>(source: PlanModuleEntitlementSource<TEnabledModule>): TEnabledModule[] {
  ...
  getPermittedEnabledModules<TEnabledModule extends EntitledModuleRecord>(
    ...
    return enabledModules.filter((item) => this.hasModulePermission(item.module, permissionSet, hasAdminModuleAccess));
  }

  hasModulePermission(module: EnabledModuleRecord, permissionSet: Set<string>, hasAdminModuleAccess: boolean): boolean {
    return (
      hasAdminModuleAccess ||
      module.permissions.some((permission) =>
        Object.values(PermissionAction).some((action) => permissionSet.has(this.buildPermissionKey(permission.code, action))),
      )
    );
  }

  async getCompanyAllowedModuleIds(companyId: number, client?: EntitlementQueryClient): Promise<Set<number>> {
    return EntitlementService.getCompanyAllowedModuleIds(this.getQueryClient(client), companyId);
  }

  async getCompanyAllowedModules(companyId: number, client?: EntitlementQueryClient): Promise<CompanyPlanModule[]> {
    return EntitlementService.getCompanyAllowedModules(this.getQueryClient(client), companyId);
  }

  async getCompanyPlanSidebarItems(companyId: number, permittedModuleIds: Set<number>, client?: EntitlementQueryClient): Promise<CompanyPlanSidebarItem[]> {
    return EntitlementService.getCompanyPlanSidebarItems(this.getQueryClient(client), companyId, permittedModuleIds);
  }

  static async getCompanyAllowedModuleIds(client: EntitlementQueryClient, companyId: number): Promise<Set<number>> {
    const rows = await this.getLatestCompanyPlanModuleRows(client, companyId);

    return new Set(rows.map((row) => row.moduleId));
  }

  static async getCompanyAllowedModules(client: EntitlementQueryClient, companyId: number): Promise<CompanyPlanModule[]> {
    const rows = await this.getLatestCompanyPlanModuleRows(client, companyId);
  ...
  static async getCompanyPlanSidebarItems(
    ...
    for (const planSystem of subscription?.plan.systems ?? []) {
      for (const item of planSystem.system.sidebarItems) {
        if (item.itemType === 'LINK' && (!item.moduleId || !permittedModuleIds.has(item.moduleId))) {
          continue;
        }
  ...
  private static async getLatestCompanyPlanModuleRows(client: EntitlementQueryClient, companyId: number) {
    const subscription = await client.companySubscription.findFirst({
      where: {
        companyId,
        status: {
    ...
    return subscription?.plan.systems.flatMap((planSystem) => planSystem.system.modules) ?? [];
  }

  private getLatestSubscriptionPlanModules<TEnabledModule extends EntitledModuleRecord>(source: PlanModuleEntitlementSource<TEnabledModule>): TEnabledModule[] {
    ...
    return subscription.plan.systems.flatMap((planSystem) => (planSystem.system.modules ?? []).filter((item) => this.isEnabledModuleUsable(item)));
  }

  private dedupeModules<TEnabledModule extends EntitledModuleRecord>(modules: TEnabledModule[]): TEnabledModule[] {
